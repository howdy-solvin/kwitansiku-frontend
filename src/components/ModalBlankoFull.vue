...
export default {
  ...
  setup() {
    const blankoFullSection = ref(null)

    const printModal = () => {
      blankoFullSection.value.classList.add('enable-print-view')
      window.print()
    }
    return { printModal, blankoFullSection }
  }
}
</script>
  ...
  >
    ...
    >
      ...
      <section class="overflow-y-auto flex flex-col gap-4">
        <div class="bg-slate-300 p-6 relative" ref="blankoFullSection" id="kwitansi-section">
          <slot name="banner" />
          <slot name="main" />
        </div>
      </section>
